...
void Application::Setup()
{
	...
	GLuint samplerID = glGetUniformLocation(triangle.shader, "samp");
	if (samplerID > 0) {
		triangle.samplers.push_back(samplerID);
	}

	samplerID = glGetUniformLocation(triangle.shader, "samp2");
	if (samplerID > 0) {
		triangle.samplers.push_back(samplerID);
	}

	GLint id = loadTexture("Assets/Cobble_Stone.jpg");
	if (id > 0) {
		triangle.textureIDs.push_back(id);
	}

	id = loadTexture("Assets/Bowser.png");
	if (id > 0) {
		triangle.textureIDs.push_back(id);
	}
}
...
void Application::Draw()
{
	...
	glBindTexture(GL_TEXTURE_2D, triangle.textureIDs[0]);
	glUniform1i(triangle.samplers[0], 0);
	glActiveTexture(GL_TEXTURE0);

	glBindTexture(GL_TEXTURE_2D, triangle.textureIDs[1]);
	glUniform1i(triangle.samplers[1], 1);
